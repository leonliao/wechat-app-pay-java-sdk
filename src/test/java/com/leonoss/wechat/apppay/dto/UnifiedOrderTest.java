...
public class UnifiedOrderTest {
  ...
  public void testUnifiedOrderCases() {
    ...
    // 设置商户Key
    order.setSign(Util.validateFieldsAndGenerateWxSignature(order, key));
    System.out.println(order.toString());

    assertEquals("FCFAC3E09969AF7AF5423EF1BFB02F3C", order.getSign());
    System.out.println(WechatAppPayProtocolHandler.marshalToXml(order, key));
    ...
    try {
      Util.validateFieldsAndGenerateWxSignature(order, key);
  ...
  public void testUnmarshall() throws MalformedPduException {
    String xml = "<xml>" + "	  	<appid>wx333386e333333c44</appid>" + "	  	<attach>说明</attach>"
        + "	  	<body>Ipad mini 16G 白色</body>" + "	  	<detail>Ipad mini 16G 白色 二手</detail>"
        + "	  	<device_info>013467007045764</device_info>" + "	  	<fee_type>CNY</fee_type>"
        + "	 	<goods_tag>WXG</goods_tag>" + "	  	<limit_pay>limit_pay</limit_pay>"
        ...
        + "	  	<spbill_create_ip>8.8.8.8</spbill_create_ip>"
        + "	  	<time_expire>20091227091010</time_expire>"
        + "	  	<time_start>20091225091010</time_start>" + "	  	<total_fee>888</total_fee>"
        + "	  	<trade_type>APP</trade_type>"
        + "	  	<sign>FCFAC3E09969AF7AF5423EF1BFB02F3C</sign></xml>";

    UnifiedOrder order = WechatAppPayProtocolHandler.unmarshalFromXmlAndValidateSignature(xml,
        UnifiedOrder.class, key);
