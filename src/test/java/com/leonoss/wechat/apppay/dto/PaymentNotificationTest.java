...
public class PaymentNotificationTest {
  ...
  public void testPaymentNotificationCases() {
    ...
    notification.setSign(Util.validateFieldsAndGenerateWxSignature(notification, key));
    System.out.println(notification.toString());

    assertEquals("9973E73A7F4EF10536C7FBBA79EF673E", notification.getSign());
    System.out.println(WechatAppPayProtocolHandler.marshalToXml(notification, key));

    try {
      notification.setAppid(null);

      Util.validateFieldsAndGenerateWxSignature(notification, key);
  ...
  public void testUnmarshall() throws MalformedPduException {
    String receivedXml = "<xml>" + "	<appid>wx333386e333333c44</appid>" + "	<attach>说明</attach>"
        + "	<bank_type>bank_type</bank_type>" + "	<cash_fee>10</cash_fee>"
        + "	<cash_fee_type>CNY</cash_fee_type>" + "	<coupon_count>2</coupon_count>"
        + "	<coupon_fee>20</coupon_fee>" + "	<coupon_fee_1>10</coupon_fee_1>"
        + "	<coupon_fee_2>10</coupon_fee_2>" + "	<coupon_id_1>c1</coupon_id_1>"
        + "	<coupon_id_2>c2</coupon_id_2>" + "	<device_info>013467007045764</device_info>"
        + "	<err_code>err_code</err_code>" + "	<err_code_des>err_code_des</err_code_des>"
        + "	<fee_type>CNY</fee_type>" + "	<is_subscribe>Y</is_subscribe>"
        + "	<mch_id>1611111110</mch_id>" + "	<nonce_str>nounce</nonce_str>"
        + "	<openid>openid</openid>" + "	<out_trade_no>1217752501201407033233368018</out_trade_no>"
        + "	<result_code>SUCCESS</result_code>" + "	<return_code>return_code</return_code>"
        + "	<return_msg>return_msg</return_msg>" + "	<time_end>20091227091010</time_end>"
        + "	<total_fee>888</total_fee>" + "	<trade_type>APP</trade_type>"
        + "	<transaction_id>transaction_id</transaction_id>"
        + "	<sign>9973E73A7F4EF10536C7FBBA79EF673E</sign></xml>";
    PaymentNotification notification = WechatAppPayProtocolHandler
        .unmarshalFromXmlAndValidateSignature(receivedXml, PaymentNotification.class, key);
