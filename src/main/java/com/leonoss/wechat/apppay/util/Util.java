...
public abstract class Util {
	 ...
	 * @return
	 */
	public static String validateFieldsAndGenerateWxSignature(
			WeixinPaySignablePdu valueObject) {
		if (valueObject.getSecret() == null) {
			throw new IllegalStateException(
					"Secret is not set before computing the signature.");
		}
		...
		logger.debug("The input to sign before attaching the secret: "
				+ signInput);
		signInput += "&key=" + valueObject.getSecret();
		logger.debug("The input to sign after attaching the secret: "
				+ signInput);
		String md5Digest;
		try {
			md5Digest = Util.bytesToHexString(
					Util.md5(signInput.getBytes("UTF-8"))).toUpperCase();
